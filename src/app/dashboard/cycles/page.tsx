  ...
  return (
    <>
      <PageHeader
        ...
        breadcrumb={[{ label: 'Dashboard', href: '/dashboard' }, { label: 'Cycles' }]}
      />
      ...
      <Card className="relative overflow-hidden mb-6">
        ...
        <CardBody className="relative p-5 sm:p-7">
          <div className="flex items-center gap-2.5 mb-3">
            ...
            <div>
              <h2 className="text-[18px] font-bold text-fg">What is a Cycle?</h2>
              <p className="text-[12.5px] text-fg-muted">A complete earning period, start to finish.</p>
            </div>
          </div>
          <p className="text-[14px] leading-relaxed text-fg-muted max-w-2xl">
            A <span className="text-fg font-semibold">Cycle</span> is a fixed-duration earning period. You allocate
            ...
            <span className="text-fg font-semibold">every day</span> the Cycle runs. When it reaches maturity, your
            principal returns to your balance — ready to withdraw or roll into a new Cycle.
          </p>

          <div className="mt-5 grid sm:grid-cols-3 gap-3">
            <Fact icon={<TrendingUp className="size-4" />} title="Earnings credited daily"
                  body="Your chosen tier's daily rate is added to your balance every day the Cycle runs." />
            ...
            <Fact icon={<Wallet className="size-4" />} title="Principal returns at maturity"
                  body="When the Cycle completes, your principal unlocks — withdraw it or start a new Cycle." />
          </div>
        </CardBody>
      </Card>
      ...
      {/* ── How it works ─────────────────────────────────────────── */}
      <Card className="mb-6">
        <CardHeader title="How a Cycle works" icon={<RefreshCw className="size-4" />}
      ...
      <Card className="mb-6">
        ...
        <CardBody className="pt-1 grid sm:grid-cols-2 gap-x-8 gap-y-3">
          {[
            ['When are earnings paid?', 'Daily — the day’s profit is credited automatically each evening while the Cycle is active.'],
            ['Can I withdraw mid-cycle?', 'Your principal is locked until the Cycle matures. Earned profit accrues along the way.'],
            ['What decides my rate?', 'Each access tier sets its own daily rate and duration. Higher tiers run shorter, higher-rate cycles.'],
            ['What happens at maturity?', 'It settles automatically — your capital returns to your balance and the Cycle closes. No action needed; you don’t cash out manually.'],
